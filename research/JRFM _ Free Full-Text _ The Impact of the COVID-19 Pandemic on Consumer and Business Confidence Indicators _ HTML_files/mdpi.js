...
{$(this).prop("src",$(this).data("src"));});$(this).find("span").toggle();abstractImagePreview.toggle();$(document).foundation('equalizer','reflow');});$('.popupgallery').each(function(){$(this).magnificPopup({type:'image',delegate:'a',index:2,image:{verticalFit:false},gallery:{enabled:true}});});$(document).on("click",'.js-browse-figures .openpopupgallery',function(){var target=$(this).data('target');$('#'+target+'.popupgallery').magnificPopup('open');});$(document).on("click",'.abstract-image-preview .openpopupgallery',function(){var target=$(this).data('target');var index=$(this).data('imgindex');$('#'+target+'.popupgallery').magnificPopup('open');$('#'+target+'.popupgallery').magnificPopup('goTo',index);});$(document).on("click",".rss-link",function(e){e.preventDefault();$('#rssNotificationModal .js-rss-notification-confirm').attr("href",$(this).attr("href"));$('#rssNotificationModal').foundation('reveal','open');});$("table").on("keyup",".filters .filter",function(){setTimeout(function(){$(document).foundation('equalizer','reflow');},200);});$(window).scroll(function(){if($(this).scrollTop()>220){$('.back-to-top').fadeIn(500);}else{$('.back-to-top').fadeOut(500);}});$('.back-to-top').click(function(event){event.preventDefault();$('html, body').animate({scrollTop:0},500);return false;})
...
$.scrollToElement=function(element,speed){if(speed==null){speed=500;}
return $('html, body').animate({scrollTop:element.offset().top},speed);};$.fn.extend({scrollToError:function(){var formFirstError;formFirstError=$(this).find('.element.error:visible').first();if(formFirstError&&formFirstError.length>0){return $.scrollToElement(formFirstError);}}});